...
public class Rain
{
    ...
    public void startRain(int x,ArrayList<Tarp> listLonas,int realY){
        ...
        for (int i=0; i< listLonas.size();i++){                   
            while  (j!= realY ){ 
                ...
                if (listLonas.get(i).verificarHueco(x,j)){
                    j=j+1;Gota lluvia = new Gota (x,j);punto[0]=x;punto[1]=j;lluviaCompleta[j]=punto;gotasDeLluvia.add(lluvia) ;break; 
                }else{
                    if (x <= listLonas.get(i).getPuntoDos()[0] && x >= listLonas.get(i).getPuntoUno()[0]){                   
                        ...
                        if (realY-j==(int) k){                                                                                                                                              
                            if ((float) listLonas.get(i).getPendiente()>0){
                                 x=x-1;j=j-1;Gota lluvia = new Gota (x,j);punto[0]=x;punto[1]=j;lluviaCompleta[j]=punto;gotasDeLluvia.add(lluvia);
        ...
        while  (j!= realY ){
            int[] punto = new int[2];j=j+1;Gota lluvia = new Gota (x,j);punto[0]=x;punto[1]=j;lluviaCompleta[j]=punto;gotasDeLluvia.add(lluvia);
        }
        lluviaX=x;
    }
